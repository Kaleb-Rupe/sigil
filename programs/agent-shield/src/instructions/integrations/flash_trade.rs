...
pub fn verify_flash_trade_instruction(ix_data: &[u8]) -> Result<()> {
    require!(
        ix_data.len() >= 8,
        AgentShieldError::InvalidFlashTradeInstruction
    );
    ...
    require!(is_known, AgentShieldError::InvalidFlashTradeInstruction);

    // Read priceWithSlippage.price (u64 at offset 8, after discriminator)
    let price_bytes: [u8; 8] = ix_data[8..16]
        .try_into()
        .map_err(|_| error!(AgentShieldError::InvalidFlashTradeInstruction))?;
    let price = u64::from_le_bytes(price_bytes);

    require!(price > 0, AgentShieldError::FlashTradePriceZero);
