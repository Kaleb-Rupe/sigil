...
describe("Kamino API Client", () => {
  ...
  describe("Configuration", () => {
    ...
    it("enforces HTTPS for non-localhost URLs", () => {
      expect(() =>
        configureKaminoApi({ baseUrl: "http://api.kamino.finance" }),
      ).to.throw("HTTPS");
    });

    it("allows http://localhost for testing", () => {
      expect(() =>
        configureKaminoApi({ baseUrl: "http://localhost:8080" }),
      ).to.not.throw();
    });

    it("allows http://127.0.0.1 for testing", () => {
      expect(() =>
        configureKaminoApi({ baseUrl: "http://127.0.0.1:8080" }),
      ).to.not.throw();
    });

    it("strips trailing slashes from baseUrl", () => {
      configureKaminoApi({ baseUrl: "https://api.kamino.finance/" });
      expect(getKaminoApiConfig().baseUrl).to.equal(
...
describe("Kamino Verification", () => {
  describe("Program ID check", () => {
    it("accepts instructions with allowed programs", () => {
      const ixs = [
        buildKaminoIx(DEPOSIT_OBLIGATION_COLLATERAL_DISCRIMINATOR, 1000n),
      ];
      expect(() =>
        verifyKaminoInstructions(ixs, "deposit", 1000n, FAKE_VAULT),
      ).to.not.throw();
    });
    ...
    it("rejects instructions with unexpected programs", () => {
      const ixs: Instruction[] = [
        {
          programAddress:
            "BADPROGRAM1111111111111111111111111111111" as Address,
          accounts: [
            { address: FAKE_VAULT, role: AccountRole.WRITABLE_SIGNER },
          ],
  ...
  describe("Discriminator check", () => {
    ...
    it("fails when discriminator mismatches", () => {
      ...
      const ixs = [
        buildKaminoIx(BORROW_OBLIGATION_LIQUIDITY_DISCRIMINATOR, 1000n),
      ];
      expect(() =>
        verifyKaminoInstructions(ixs, "deposit", 1000n, FAKE_VAULT),
    ...
    it("validates all 4 KLend discriminators", () => {
      ...
      for (const [disc, action] of cases) {
        ...
        expect(() =>
          verifyKaminoInstructions(ixs, action, 1000n, FAKE_VAULT),
        ).to.not.throw();
      }
    });
  });
  ...
  describe("Amount check", () => {
    it("passes when amount matches", () => {
      const ixs = [
        buildKaminoIx(DEPOSIT_OBLIGATION_COLLATERAL_DISCRIMINATOR, 500_000n),
      ];
      expect(() =>
        verifyKaminoInstructions(ixs, "deposit", 500_000n, FAKE_VAULT),
      ).to.not.throw();
    });
    ...
    it("fails when amount mismatches", () => {
      ...
      expect(() =>
        verifyKaminoInstructions(ixs, "deposit", 999_999n, FAKE_VAULT),
    ...
    it("skips amount check for zero expected amount", () => {
      ...
      expect(() =>
        verifyKaminoInstructions(ixs, "deposit", 0n, FAKE_VAULT),
      ).to.not.throw();
    });
  });
  ...
  describe("Signer check", () => {
    it("passes when vault is signer", () => {
      const ixs = [
        buildKaminoIx(DEPOSIT_OBLIGATION_COLLATERAL_DISCRIMINATOR, 1000n, true),
      ];
      expect(() =>
        verifyKaminoInstructions(ixs, "deposit", 1000n, FAKE_VAULT),
      ).to.not.throw();
  ...
  describe("Empty instructions", () => {
    it("throws for zero instructions", () => {
      expect(() =>
        verifyKaminoInstructions([], "deposit", 1000n, FAKE_VAULT),
