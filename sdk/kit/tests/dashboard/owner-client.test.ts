...
import { OwnerClient, type OwnerClientConfig } from "../../src/dashboard/index.js";
...
describe("OwnerClient", () => {
  describe("constructor", () => {
    ...
    it("throws if rpc missing", () => {
      expect(() => new OwnerClient({ ...validConfig(), rpc: undefined as any }))
        .to.throw("rpc is required");
    });

    it("throws if vault missing", () => {
      expect(() => new OwnerClient({ ...validConfig(), vault: undefined as any }))
        .to.throw("vault is required");
    });

    it("throws if owner missing", () => {
      expect(() => new OwnerClient({ ...validConfig(), owner: undefined as any }))
        .to.throw("owner is required");
    });

    it("throws if network missing", () => {
      expect(() => new OwnerClient({ ...validConfig(), network: undefined as any }))
  ...
  describe("method count", () => {
    it("has 6 read methods", () => {
      ...
      const reads = ["getVaultState", "getAgents", "getSpending", "getActivity", "getHealth", "getPolicy"];
      for (const name of reads) {
        expect(typeof (client as any)[name]).to.equal("function", `${name} should be a function`);
      }
    });
    ...
    it("has 23 mutation methods", () => {
      const client = new OwnerClient(validConfig());
      const mutations = [
        "freezeVault", "resumeVault", "closeVault", "syncPositions",
        "deposit", "withdraw",
        "addAgent", "pauseAgent", "unpauseAgent", "revokeAgent",
        "queueAgentPermissions", "applyAgentPermissions", "cancelAgentPermissions",
        "queuePolicyUpdate", "applyPendingPolicy", "cancelPendingPolicy",
        "createConstraints", "queueConstraintsUpdate", "applyConstraintsUpdate", "cancelConstraintsUpdate",
        "queueCloseConstraints", "applyCloseConstraints", "cancelCloseConstraints",
      ];
